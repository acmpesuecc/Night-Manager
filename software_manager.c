...
void fetch_available_packages(GtkListStore *list_store) {
    ...
    // Use 'apt list --installed' to get a list of installed packages
    fp = popen("apt list --installed", "r");
    if (fp == NULL) {
        perror("Failed to run apt command");
        return;
    }
...
void on_install_clicked(GtkWidget *widget, gpointer data) {
    ...
    const char *command[] = { "/bin/sh", "-c", "echo 'Installing package...' && sudo apt install -y <package-name>", NULL };
...
void on_uninstall_clicked(GtkWidget *widget, gpointer data) {
    ...
    const char *command[] = { "/bin/sh", "-c", "echo 'Uninstalling package...' && sudo apt remove -y <package-name>", NULL };
